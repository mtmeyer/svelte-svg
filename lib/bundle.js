...
async function convertSVGToSvelte() {
    ...
    try {
        ...
        console.log(source.magenta(`--- No SVG's in the current directory ---`));
    }
    if (svgs.length > 0) {
        const outputDir = path__default['default'].join(CURR_DIR, 'output');
        // Check if output directory already exists
        let doesDirAlreadyExist = false;
        await fs__default['default'].readdir(outputDir, (err, dir) => {
            if (dir)
                doesDirAlreadyExist = true;
        });
        // If no output directory exists, create one
        if (!doesDirAlreadyExist) {
            await fs__default['default'].mkdir(outputDir, (err) => {
                if (err)
                    throw err;
            });
        }
        ...
        console.log(source.green(`Successfully created`, source.bold(svgs.length), `Svelte components`));
    }
}
